...
class ApisStaticsManager {
  ...
  static Future<SourceResponseDM> getSources(String categoryId) async {
    Uri url = Uri.https(apiNewsBaseUrl, apiNewsSourcesUrl,
        {"apiKey": apiNewsKey, "category": categoryId});
  ...
  static Future<ArticlesResponseDM> getArticle(
      {String? sourceId, String? searchKeyWord}) async {
    Uri url = Uri.https(apiNewsBaseUrl, "v2/everything",
        {"sources": sourceId, "apiKey": apiNewsKey, "q": searchKeyWord});
    Response response = await get(url);
